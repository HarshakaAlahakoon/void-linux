...
Main Reference: https://gist.github.com/itzk0tlin/91d1368cb4b366c039508da4e31d04d7
...
bash-5.2# 
  ...
  19. Post-Installation Setup
      ...
      Packages:
        ...
        chromium
        chromium-widevine
        # Erlang build dependancies ==== <start>
        apache-fop
        ncurses-devel
        openssl-devel
        MesaLib-devel
  ...
  22. Dev setups:
      ...
      - Setup Python environment:
        ...
        ```
      - Setup `asdf` for managing multiple language versions:
        ```
        ...
        mkdir /smithy/.asdf
        ```
        Add the following to ~/.config/fish/config.fish (if not already present):
        ```fish
        ...
        - Setup JDK 11 (needed for Erlang build):
          We previously installed openjdk11, but need to set it up in fish shell path.
          Add the following to ~/.config/fish/config.fish:
          ```fish
          set -gx PATH /usr/lib/jvm/openjdk11/bin $PATH

          ```

        - Setup Erlang and Elixir:
        ```
        # Erlang build process looks for wx-config, but we only have wx-config-gtk3
        # Therefore, first check if wx-config exists:
        which wx-config
        # If it does not exist, create a symlink to wx-config-gtk3
        sudo ln -s /usr/bin/wx-config-gtk3 /usr/local/bin/wx-config

        asdf plugin add erlang
        asdf install erlang 28.0    # Or any other version you need
        # Set the global version of Erlang (optional)
        asdf set -u erlang 28.0

        asdf plugin add elixir
        asdf install elixir 1.18.4-otp-28   # Or any other version you need, but make sure it matches the Erlang version
        # Set the global version of Elixir (optional)
        asdf set -u elixir 1.18.4-otp-28

        ```


  23. Setup 1Password and Brave browser (optional, as normal user)
      ...
      If the 1password icon is not shown in start menu, dmenu etc. then edit the desktop file and set the icon property to:
      ```
      ...
      Zen Browser Setup:
        TODO: Create/Fix xbps-src template file for this.
        Dowload the AppImage from github, and place it in `/opt/zen-browser/`.
        Copy a logo from "https://github.com/zen-browser/desktop/tree/dev/configs/branding/release" to the same install destination.
        Create `/usr/share/applications/zen-browser.desktop` file and add the following to it:
        ```
        [Desktop Entry]
        Name=Zen Browser
        Exec=/opt/zen-browser/zen-x86_64.AppImage
        Icon=/opt/zen-browser/logo.png
        Type=Application
